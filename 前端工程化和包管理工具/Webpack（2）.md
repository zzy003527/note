...
- 如果多入口文件中都引用了同一份代码，我们不希望这份代码被打包到两个文件中，导致代码重复，体积更大。
  ...
  我们需要**提取多入口的重复代码，只打包生成一个 js 文件**，其他文件引用它就好
...
- webpack 会先**从左到右执行** loader 链中的每个 loader 上的 **pitch 方法（如果有）**，然后**再从右到左执行 loader 链**中的每个 loader 上的**普通 loader 方法**。
...
##### 4.1.4.6 手写clean-log-loader

- 作用：用来清理js代码中的`console.log`

- ```js
  // loaders/clean-log-loader.js
  module.exports = funciton cleanLogLoader(content) {
  	//将console.log替换为空
...
##### 4.1.4.8 手写babel-loader

- 作用：编译js代码，将ES6+语法编译成ES5语法
...
- loader配置
  ...
  ```js
  {
    ...
    type: "javascript/auto", // 解决图片重复打包问题（阻止webpack默认处理图片资源，只使用file-loader处理）
  },
  ```
...
- **最简单的插件**
  ...
  - plugins/test-plugin.js
    ...
    class TestPlugin {
      ...
      // 1. webpack加载webpack.config.js中所有配置，此时就会new TestPlugin() ，会执行插件 constructor 方法
...
- **注册hooks**
  ...
  - `compliation钩子`需要在`compiler.hooks.make`中注册
...
- **启动调试**
  ...
  - 打开 Chrome 浏览器，F12 打开浏览器调试控制台。
    ...
    - 此时控制台会显示一个绿色的图标![调试控制台](https://yk2012.github.io/sgg_webpack5/imgs/source/debug.png)
  ...
  class CleanWebpackPlugin {
    ...
    removeFiles(fs, path) {
      try {
        ...
        // 遍历文件，删除
          // 需要判断是文件还是文件夹，如果是文件夹，就得删除下面所有文件，才能删除文件夹
        files.forEach((file) => {
          // 获取文件完整路径
  ...
  class InlineChunkWebpackPlugin {
    ...
    apply(compiler) {
      compiler.hooks.compilation.tap("InlineChunkWebpackPlugin", (compilation) => {
          // 1.得到HtmlWebpackPlugin独有的hooks
        const hooks = HtmlWebpackPlugin.getHooks(compilation);
  
          // 2.注册 htmlwebpackplugin的alterAssetTagGroups钩子
...
- 学习了 webpack 中的 loader 和 plugin 用法。包括是什么，怎么使用，如何自定义等。
